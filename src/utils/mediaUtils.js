...
const defaultImage = 'https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&w=800&q=80';

export function getSuggestedMedia(title = '', description = '', userImage = '', userVideo = '') {
  ...
  else if (/(pizza|cuisine|pâtisserie|patisserie|pâte|pate|recette|robot pâtissier)/.test(text)) themeKey = 'cuisine';
  ...
  else if (/(plomberie|perceuse|marteau|bricolage|électrique|electricite|nettoyeur|kärcher|karcher|ponceuse)/.test(text)) themeKey = 'bricolage';
  else if (/(smartphone|écran cassé|ecran casse|electronique|ordinateur|macbook|pc|python|code|informatique|développement|developpement|script)/.test(text)) themeKey = 'tech';
...
export function getFallbackImage(category = '', title = '') {
  ...
  return getSuggestedMedia(title, category).image || fallbackCategoryImages.default;
}
